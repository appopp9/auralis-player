...
class MusicRepository @Inject constructor(
    ...
    val recentlyAdded: StateFlow<List<Song>> = songs
        .map { list -> list.sortedWith(recentlyAddedComparator).take(60) }
        .stateIn(scope, SharingStarted.WhileSubscribed(5_000), emptyList())
    ...
    /** Tracks you played before but haven't touched in 30+ days — for rediscovery. */
    val forgotten: StateFlow<List<Song>> = songs
        .map { list ->
            val cutoff = System.currentTimeMillis() - 30L * 86_400_000L
            list.filter { it.playCount > 0 && it.lastPlayedAt in 1 until cutoff }
                .sortedBy { it.lastPlayedAt }
                .take(14)
        }
        .stateIn(scope, SharingStarted.WhileSubscribed(5_000), emptyList())

    /** Groups of tracks that look like duplicates (same title + artist). */
    val duplicates: StateFlow<List<List<Song>>> = songs
        .map { list ->
            list.groupBy { "${it.title.lowercase().trim()}|${it.displayArtist.lowercase().trim()}" }
                .values
                .filter { it.size > 1 }
    ...
    /** Increments the play counter once (threshold reached). */
    suspend fun incrementPlayCount(songId: Long) {
        songDao.incrementPlayCount(songId, System.currentTimeMillis())
    }

    /**
     * Records a stretch of real listening time. Only called with the actual
     * milliseconds spent in the playing state, so pauses/seeks never inflate
     * the total. Ignores non-positive deltas to avoid double counting.
     */
    suspend fun addListeningTime(songId: Long, playedMs: Long) {
        if (playedMs <= 0L) return
        historyDao.insert(
            PlayHistoryEntity(songId = songId, playedAt = System.currentTimeMillis(), playedMs = playedMs)
        )
    }
    ...
    companion object {
        /**
         * MediaStore's DATE_ADDED is unreliable on its own:
         *  - Files copied/restored in bulk share one identical second, so a
         *    plain sort leaves them in whatever order the DB returned them.
         *  - Re-indexed or side-loaded files often report 0.
         *  - A few OEMs report milliseconds instead of seconds.
         * Normalise all three cases so "recently added" behaves like every
         * other music player.
         */
        private fun addedSeconds(song: Song): Long {
            // Anything past this in "seconds" is really a millisecond value.
            val msThreshold = 100_000_000_000L
            fun norm(v: Long) = if (v > msThreshold) v / 1000L else v
            val added = norm(song.dateAddedSec)
            if (added > 0L) return added
            return norm(song.dateModifiedSec)
        }

        /**
         * Newest first, with the MediaStore row id as the tie-breaker. Ids grow
         * monotonically as files are indexed, so songs imported within the same
         * second keep a stable, genuinely chronological order instead of
         * shuffling around every time the library reloads.
         */
        val recentlyAddedComparator: Comparator<Song> =
            compareByDescending<Song> { addedSeconds(it) }
        ...
        fun sort(songs: List<Song>, order: SortOrder): List<Song> = when (order) {
            ...
            SortOrder.RECENTLY_ADDED -> songs.sortedWith(recentlyAddedComparator)
            SortOrder.RECENTLY_PLAYED -> songs.sortedWith(
                compareByDescending<Song> { it.lastPlayedAt }.thenByDescending { it.id }
            )
            SortOrder.MOST_PLAYED -> songs.sortedWith(
                compareByDescending<Song> { it.playCount }
                    .thenByDescending { it.lastPlayedAt }
                    .thenBy { it.title.lowercase(Locale.ROOT) }
            )
            SortOrder.DURATION -> songs.sortedWith(
                compareByDescending<Song> { it.durationMs }.thenBy { it.title.lowercase(Locale.ROOT) }
            )
            SortOrder.YEAR -> songs.sortedWith(
                compareByDescending<Song> { it.year }
                    .thenBy { it.displayAlbum.lowercase(Locale.ROOT) }
                    .thenBy { it.discNumber }
                    .thenBy { it.trackNumber }
            )
            SortOrder.ARTIST -> songs.sortedWith(
                compareBy<Song>({ it.displayArtist.lowercase(Locale.ROOT) })
                    .thenBy { it.displayAlbum.lowercase(Locale.ROOT) }
                    .thenBy { it.discNumber }
                    .thenBy { it.trackNumber }
                    .thenBy { it.title.lowercase(Locale.ROOT) }
            )
            SortOrder.ALBUM -> songs.sortedWith(
                compareBy<Song>({ it.displayAlbum.lowercase(Locale.ROOT) })
                    .thenBy { it.discNumber }
                    .thenBy { it.trackNumber }
                    .thenBy { it.title.lowercase(Locale.ROOT) }
            )
