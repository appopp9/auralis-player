...
fun AuralisAppScaffold(
    ...
    val songActions = SongActions(
        ...
        onViewAlbum = { song -> navController.navigate(Routes.album(song.albumId)) },
        onViewArtist = { song -> navController.navigate(Routes.artist(song.artist)) },
        onEditTags = { song -> navController.navigate(Routes.tagEditor(song.id)) },
        onSetRingtone = { song ->
            if (mainViewModel.canSetRingtone()) {
                mainViewModel.setAsRingtone(song)
            } else {
