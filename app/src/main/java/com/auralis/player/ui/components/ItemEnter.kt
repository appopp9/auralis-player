 ...
 *
 * Crucially, this animates **only** while the screen is inside its entrance
 * window (the first [WINDOW_MS] after navigation). Rows composed later — which
 * is exactly what happens while scrolling — get a plain, un-layered modifier,
 * so fast scrolling stays completely free of animation and graphics-layer cost.
 */
fun Modifier.appear(
    index: Int = 0,
    distance: Dp = 18.dp,
    ...
    if (!inWindow) {
        ...
        graphicsLayer {
            ...
            // A very slight scale-up makes rows feel like they settle into
            // place instead of merely sliding.
