...
class AuralisBitmapLoader(
    ...
    /**
     * Mirrors the "artwork in notification" setting. When it is off the loader
     * reports no artwork, which makes the media notification and lock screen
     * fall back to the plain text layout.
     */
    @Volatile
    var artworkEnabled: Boolean = true
    ...
    override fun loadBitmap(uri: Uri): ListenableFuture<Bitmap> {
        if (!artworkEnabled) {
            return Futures.immediateFailedFuture(IllegalStateException("Artwork disabled"))
        }
